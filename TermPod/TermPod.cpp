...
{
#if defined(_WIN32)
	SetConsoleTitleA("TermPod (By Malte0621) v0.3.1");
#endif
	std::cout << "TermPod (By Malte0621) v0.3.1" << std::endl;
		...
		for (const auto& direntry : std::filesystem::recursive_directory_iterator(args.dir)) {
			...
			switch (type) {
				...
				case POD5:
					...
					break;
				case POD6:
					if (compression_pod6 && buffer.size() >= 2048 && extension != ".smp" && extension != ".fxa" && extension != ".fxe" && extension != ".png") {
